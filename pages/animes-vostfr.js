...
const ListAnime = () => {
  ...
  if (page === undefined){
   ...
   useEffect(() => {
    if (url === ""){
      url="https://api.toonanime.org/api/animefilter?page="+page+"&limite=20&status=&annee=&language=vostfr&sort=&genres=&title=&type="
    }
    setUrl(url)
    setLoading(true)
  ...
  const handleChange = async (e) => {
    ...
    if (e.target.id==="title"){
      ...
      router.push(`/animes-vostfr`, `/animes-vostfr?page=1`)
      setCurrentpage(1)
  ...
  const handlePage = async (e) => {
   ...
   console.log(e.target)
    ...
    router.push(`/animes-vostfr`, `/animes-vostfr?page=${e.selected+1}`)
...
var truncate = (str, max, suffix) => {
  return str.length < max
    ...
    return (
      <>
  <NextSeo
      title="Liste des Animes Vostfr  - ToonAnime"
      description="Liste des Animes Vostfr  - ToonAnime"
      additionalMetaTags={[{
        name: 'viewport',
      ...
      }, 
      {
        ...
        name: 'keywords',
        content: 'liste anime vostfr ,anime vostfr streaming,toonanime'
      },{
    ...
    />
        ...
        <h1 className="text-2xl uppercase pl-5 ">animes Vostfr</h1>
